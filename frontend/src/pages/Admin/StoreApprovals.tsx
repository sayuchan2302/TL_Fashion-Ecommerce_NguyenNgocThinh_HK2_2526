...
const mapStore = (store: StoreProfile, index: number): ManagedStore => ({
  ...
  operatingStatus:
    store.approvalStatus === 'APPROVED'
      ...
      : 'INACTIVE',
  productCount:
    store.totalOrders > 0 ? Math.max(12, Math.round(store.totalOrders * 0.18)) : index % 2 === 0 ? 24 : 12,
  liveProductCount:
    store.approvalStatus === 'APPROVED'
      ? Math.max(8, Math.round(store.totalOrders > 0 ? store.totalOrders * 0.14 : 16))
      : 0,
  responseRate:
    store.approvalStatus === 'APPROVED'
      ? Math.min(99, 88 + (index * 3) % 10)
...
const StoreApprovals = () => {
  ...
  const shareCurrentView = async () => {
    try {
      ...
      addToast('Không thể sao chép liên kết bộ lọc', 'error');
    }
  };
  ...
  const approveStores = async () => {
    ...
    try {
          ...
          prev.map((item) =>
            item.id === response.storeId
              ? {
                  ...
                  liveProductCount: Math.max(item.liveProductCount, 12),
                }
              : item,
          ),
        );
        if (detailStore?.id === response.storeId) {
          setDetailStore((current) =>
            current
              ? { ...current, approvalStatus: 'APPROVED', status: 'ACTIVE', operatingStatus: 'ACTIVE', rejectionReason: undefined }
              : null,
          );
        }
      }
      ...
      addToast(getUiErrorMessage(error, 'Phê duyệt gian hàng thất bại'), 'error');
    } finally {
  ...
  const rejectStore = async () => {
    ...
    try {
      ...
      setSelected((prev) => {
        ...
        next.delete(detailStore.id);
        return next;
      });
      addToast('Đã từ chối hồ sơ gian hàng', 'info');
    } catch (error: unknown) {
      addToast(getUiErrorMessage(error, 'Từ chối hồ sơ gian hàng thất bại'), 'error');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const applyStoreOperatingChange = async () => {
    ...
    try {
      ...
      setStores((prev) =>
        prev.map((store) =>
          confirmState.ids.includes(store.id)
            ? { ...store, operatingStatus: nextStatus, status: nextStatus }
            : store,
        ),
      );
      ...
      addToast(
        confirmState.mode === 'suspend'
          ? 'Đã tạm khóa gian hàng đã chọn'
          : 'Đã mở lại gian hàng đã chọn',
        confirmState.mode === 'suspend' ? 'info' : 'success',
      );

      setSelected(new Set());
      setConfirmState(null);
    } catch (error: unknown) {
      addToast(getUiErrorMessage(error, 'Không thể cập nhật trạng thái gian hàng'), 'error');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
      ...
      actions={(
        <>
          <div className="admin-search">
            <Search size={16} />
            <input
              placeholder="Tìm theo tên gian hàng, slug, chủ sở hữu hoặc email liên hệ"
              value={search}
              onChange={(event) => {
    ...
    >
      ...
      <PanelTabs
        ...
        activeKey={activeTab}
        onChange={(key) => {
          setActiveTab(key as StoreFilter);
          setSelected(new Set());
          setPage(1);
        }}
      />
        ...
        <div className="admin-panel">
          ...
          {!loading && filteredStores.length === 0 ? (
            <AdminStateBlock
              ...
              title={search.trim() ? 'Không tìm thấy gian hàng phù hợp' : 'Chưa có hồ sơ gian hàng'}
              description={
                search.trim()
                  ? 'Thử đổi từ khóa hoặc đặt lại bộ lọc để xem lại danh sách gian hàng.'
                  : 'Danh sách gian hàng sẽ hiển thị tại đây để quản trị viên theo dõi và xử lý.'
              }
            ...
            <>
              <div className="admin-table" role="table" aria-label="Bảng gian hàng">
                <div className="admin-table-row stores admin-table-head" role="row">
                  <div role="columnheader">
                    <input
                      type="checkbox"
                      checked={selected.size === filteredStores.length && filteredStores.length > 0}
                      onChange={(event) =>
                        setSelected(event.target.checked ? new Set(filteredStores.map((item) => item.id)) : new Set())
                      }
                    />
                  </div>
                ...
                {pagedStores.map((store) => (
                  <motion.div
                    key={store.id}
                    className="admin-table-row stores"
                    role="row"
                    whileHover={{ y: -1 }}
                    onClick={() => {
                      setDetailStore(store);
                      setRejectReason(store.rejectionReason || '');
                    }}
                  ...
                  >
                    ...
                    <div role="cell" className="store-cell">
                      ...
                      <div className="store-copy">
                        <div className="admin-bold">{store.name}</div>
                        <div className="admin-muted small">{store.slug}</div>
                      </div>
                    </div>
                    <div role="cell">
                      <div className="admin-bold">{store.applicantName || 'Chưa đăng ký chủ sở hữu'}</div>
                      <div className="admin-muted small">{store.applicantEmail || store.contactEmail || 'Chưa có email'}</div>
                    </div>
                    <div role="cell" className="store-ops-cell">
                      ...
                      <div className="admin-muted small">
                        {store.liveProductCount.toLocaleString('vi-VN')} đang bán · {store.totalOrders.toLocaleString('vi-VN')} đơn
                      </div>
                    </div>
                    <div role="cell">
                      <div className="store-status-stack">
                        <span className={`admin-pill ${approvalTone(store.approvalStatus)}`}>{approvalLabel(store.approvalStatus)}</span>
                    ...
                    <div role="cell" className="admin-actions" onClick={(event) => event.stopPropagation()}>
                      ...
                      {store.approvalStatus === 'PENDING' ? (
                        <button className="admin-icon-btn subtle" title="Duyệt gian hàng" aria-label="Duyệt gian hàng" onClick={() => openConfirm('approve', [store.id])}><Check size={16} /></button>
                      ) : null}
                      {store.approvalStatus === 'APPROVED' && store.operatingStatus === 'ACTIVE' ? (
                        <button className="admin-icon-btn subtle danger-icon" title="Tạm khóa gian hàng" aria-label="Tạm khóa gian hàng" onClick={() => openConfirm('suspend', [store.id])}><Ban size={16} /></button>
                      ) : null}
                      {store.approvalStatus === 'APPROVED' && store.operatingStatus === 'SUSPENDED' ? (
                        <button className="admin-icon-btn subtle" title="Mở lại gian hàng" aria-label="Mở lại gian hàng" onClick={() => openConfirm('reactivate', [store.id])}><RotateCcw size={16} /></button>
                      ) : null}
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="table-footer">
                <span className="table-footer-meta">
                  Hiển thị {(safePage - 1) * pageSize + 1}-{Math.min(safePage * pageSize, filteredStores.length)} trên {filteredStores.length} gian hàng
                </span>
      ...
      <AdminConfirmDialog
        ...
        description={
          confirmState?.mode === 'approve'
            ? 'Chủ sở hữu sẽ được kích hoạt quyền người bán và gian hàng chuyển sang trạng thái hoạt động.'
            : confirmState?.mode === 'suspend'
              ? 'Gian hàng sẽ bị chặn vận hành tạm thời trên sàn cho đến khi mở lại.'
              : 'Gian hàng sẽ được mở lại hoạt động và tiếp tục hiển thị trên sàn.'
        }
        selectedItems={confirmState?.selectedItems}
        selectedNoun="gian hàng"
        confirmLabel={
          actionLoading
            ? 'Đang xử lý...'
            : confirmState?.mode === 'approve'
        ...
        danger={confirmState?.mode === 'suspend'}
        ...
        onConfirm={() => {
          ...
          void applyStoreOperatingChange();
        }}
      />
            ...
            <div className="drawer-body">
              <PanelDrawerSection title="Tổng quan gian hàng">
                <div className="store-drawer-hero">
                  ...
                  <div className="store-hero-pills">
                    <span className={`admin-pill ${approvalTone(detailStore.approvalStatus)}`}>{approvalLabel(detailStore.approvalStatus)}</span>
                    <span className={`admin-pill ${operatingTone(detailStore.operatingStatus)}`}>{operatingLabel(detailStore.operatingStatus)}</span>
                  </div>
                </div>
              </PanelDrawerSection>
              ...
              <PanelDrawerSection title="Hồ sơ và chủ sở hữu">
                <div className="admin-card-list">
                  <div className="admin-card-row"><span className="admin-bold"><User size={14} style={{ verticalAlign: -2, marginRight: 6 }} /> Chủ sở hữu</span><span className="admin-muted">{detailStore.applicantName || 'Chưa đăng ký chủ sở hữu'}</span></div>
              ...
              <PanelDrawerSection title="Tín hiệu kinh doanh">
                <div className="store-signal-grid">
                  ...
                  <div className="store-signal-card"><span className="admin-muted small">Phản hồi</span><strong>{detailStore.responseRate}%</strong><span className="admin-muted small">tỷ lệ phản hồi dự tính</span></div>
              ...
              <PanelDrawerSection title="Mô tả gian hàng">
                <p className="admin-muted store-description">{detailStore.description || 'Chưa có mô tả gian hàng.'}</p>
              </PanelDrawerSection>
              ...
              <PanelDrawerSection title="Ghi chú kiểm duyệt">
                {detailStore.approvalStatus === 'PENDING' || detailStore.approvalStatus === 'REJECTED' ? (
                  ...
                  <div className="admin-card-list">
                    <div className="admin-card-row"><span className="admin-bold">Ghi chú hiện tại</span><span className="admin-muted">{detailStore.rejectionReason || 'Chưa có ghi chú kiểm duyệt. Gian hàng đang hoạt động bình thường.'}</span></div>
                  </div>
                )}
              </PanelDrawerSection>
            </div>
